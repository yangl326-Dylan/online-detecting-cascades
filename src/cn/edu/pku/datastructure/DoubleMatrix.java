...
public class DoubleMatrix implements Cloneable, java.io.Serializable {
	...
	public DoubleMatrix(int m, int n, double s) {
		this.m = m;
		this.n = n;
		A = new double[m][n];
		for (int i = 0; i < m; i++) {
	...
	public void setMatrix(int i0, int i1, int j0, int j1, DoubleMatrix X) {
		try {
			for (int i = i0; i <= i1; i++) {
				for (int j = j0; j <= j1; j++) {
	...
	public DoubleMatrix arrayTimes(DoubleMatrix B) {
		checkMatrixDimensions(B);
		DoubleMatrix X = new DoubleMatrix(m, n);
		double[][] C = X.getArray();
		for (int i = 0; i < m; i++) {
	...
	public DoubleMatrix arrayTimesEquals(DoubleMatrix B) {
		checkMatrixDimensions(B);
		for (int i = 0; i < m; i++) {
			for (int j = 0; j < n; j++) {
				A[i][j] = A[i][j] * B.A[i][j];
	...
	public DoubleMatrix times(double s) {
		DoubleMatrix X = new DoubleMatrix(m, n);
		double[][] C = X.getArray();
		for (int i = 0; i < m; i++) {
			for (int j = 0; j < n; j++) {
